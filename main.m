...
lambda=(300:50:4000)*10^-9; %freespace wavelength of incident ray in meter 
thickness=500*10^-6;  %thickness of coating in meter 
radius=100*10^-9; %radius of particle in meter 
f_v=0.04; %volume fraction. 0.01 corresponds to 1% 
...
n_pigment=tio2_n_palik_per(lambda);  %real refractive index of pigment
k_pigment=tio2_k_palik_per(lambda);  %imaginary refractive index of pigment
k_pigment(k_pigment<10^-1)=0; % this should be commented out for the real applications. the following article probably neglected low k value - https://doi.org/10.1016/j.ijheatmasstransfer.2016.08.009
n_medium=1.5*ones(length(lambda),1); %real refractive index of medium
...
plot(lambda_um,ref_lambda(:,1),'LineWidth',2)
ylim([0 1])
xlim([0 4])
legend('Reflectance','Location', 'Best')
xlabel('Wavelength [\mum]')
ylabel({'Normal to hemispherical reflectance'})
